...
/* ===================== 01 きっかけ ===================== */
{
  const s = slide(1,"位 置 づ け","「預金の増減を知りたい」を想定した、DYNATREKの活用例",
    "本インターンのテーマ ── 銀行内で保有する各種データを分析し、どのような価値に変換していくかのプロセスを学ぶ");

  const lw=6.55, ch=1.05, ys=[2.05,3.38,4.71];
  const steps=[
    ["行内のデータ","項目が多く、見たい条件でまとめて取り出す手段がない"],
    ["DYNATREK","条件を指定して、必要なデータを集計・表示できるツール"],
    ["今回の想定","例えば「預金の増減を知りたい」という問いに、どこまで答えられるか"]
  ];
  steps.forEach((st,i)=>{
    ...
    txt(s,st[1],{x:M+0.3,y:ys[i]+0.44,w:lw-0.6,h:0.58,fontSize:13.5,bold:i===2,
  ...
  txt(s,"では、実際に\nどこまで\n分かるのか",{x:rx+0.4,y:2.45,w:rw-0.8,h:1.75,
...
/* ===================== 06 結果② ATM ===================== */
{
  const s = slide(6,"結 果 ②","ATMのうち、IBでもできる取引は減っていた",
    "増加の理由が「取引全体の増加」ではないことを確かめるため、ATM側も確認");
...
/* ===================== 07 深掘り ===================== */
{
  const s = slide(7,"深 掘 り","では、どの年代でIBの利用が増えたのか",
    "「増えた」の次に知りたくなるのは「誰が使うようになったのか」。今回は年代を5つに分けて確認");
